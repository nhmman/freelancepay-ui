...
export async function POST(req: NextRequest) {
  try {
    ...
    const deliverableHash = keccak256(toHex("freelancepay-deliverable-" + jobId));
    ...
    const txHash = await waitTx(submitTx.data?.id!);

    return NextResponse.json({ success: true, txHash, deliverableHash });
  } catch (err: any) {
